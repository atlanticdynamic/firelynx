...
// TestProcessMcpAppConfig tests the simplified processMcpAppConfig function
func TestProcessMcpAppConfig(t *testing.T) {
	t.Parallel()

	t.Run("SimplifiedMcpConfiguration", func(t *testing.T) {
		// The MCP configuration uses primitive-based structure
		// All configuration is now handled by mcp-io abstraction layer
		...
		assert.Empty(t, errs, "Should not return errors for MCP config")
	})

	t.Run("NoPostprocessingRequired", func(t *testing.T) {
		// Since MCP configuration is simplified, no TOML postprocessing is needed
		app := &pbSettings.AppDefinition{
			Id: proto.String("mcp-app"),
		}
		...
		errs := processMcpAppConfig(app, appMap)
		assert.Empty(t, errs, "Should return no errors as no postprocessing is required")
	})
}
